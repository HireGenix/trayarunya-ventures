...
const ContentPostingAnimation = () => {
  ...
  return (
    <svg viewBox="0 0 400 300" width="100%" style={{ display: 'block' }} role="img" aria-label="Smart multi-platform posting">
      {/* source content card */}
      <rect x="28" y="110" width="92" height="80" rx="10" fill="rgba(255,255,255,0.04)" stroke="rgba(255,175,6,0.4)" strokeWidth="1.2" />
      <rect x="40" y="122" width="68" height="30" rx="6" fill="rgba(255,175,6,0.18)" />
      <rect x="40" y="158" width="68" height="5" rx="2" fill="rgba(255,255,255,0.4)" />
      <rect x="40" y="168" width="44" height="5" rx="2" fill="rgba(255,255,255,0.25)" />
      <text x="74" y="205" textAnchor="middle" fontSize="9" fill="rgba(255,255,255,0.5)" fontFamily="inherit">CONTENT</text>
      ...
      <motion.g animate={reduce ? {} : { scale: [1, 1.12, 1] }} transition={loop({ duration: 2 })} style={{ transformOrigin: '180px 150px' }}>
        <circle cx="180" cy="150" r="16" fill="rgba(255,255,255,0.05)" stroke="#ffaf06" strokeWidth="1.2" />
      ...
      {platforms.map((p, i) => (
        <g key={i}>
          <line x1="122" y1="150" x2={p.x - 18} y2={p.y} stroke="rgba(255,255,255,0.12)" strokeWidth="1.2" />
          {/* traveling post */}
          ...
          />
          {/* platform node */}
          <circle cx={p.x} cy={p.y} r="18" fill="rgba(255,255,255,0.05)" stroke={p.color} strokeWidth="1.5" />
