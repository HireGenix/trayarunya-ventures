...
import { PageHero, Reveal, GlowButton, ServiceIcon, TiltCard, AuroraBackground, SURFACE, TEXT, CARD } from '@/components/cinematic';
...
const ServiceDetailView = ({ service }: { service: ServiceData }) => {
  ...
  return (
    <Layout>
      ...
      {/* Summary + metrics */}
      <Box sx={{ position: 'relative', overflow: 'hidden', background: SURFACE.white, py: { xs: 8, md: 12 } }}>
        <AuroraBackground intensity={0.2} variant="light" />
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1.3fr 1fr' }, gap: { xs: 5, md: 8 }, alignItems: 'center' }}>
            <Reveal>
              ...
              <Typography sx={{ fontSize: '1.15rem', lineHeight: 1.75, color: TEXT.body }}>
                {service.summary}
              </Typography>
            </Reveal>
            <Reveal delay={0.1}>
                ...
                {service.metrics.map((m) => (
                  <TiltCard key={m.label} max={12} sx={{ borderRadius: 3 }}>
                    ...
                    >
                      ...
                      <Typography sx={{ color: TEXT.muted, fontSize: '0.74rem', mt: 0.5 }}>
                        {m.label}
                      </Typography>
                    </Box>
                  </TiltCard>
        ...
        <Container maxWidth="lg">
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3 }}>
            <Reveal direction="right">
              <Box sx={{ p: 4, height: '100%', borderRadius: 4, background: '#d92c4a0d', border: '1px solid #d92c4a33' }}>
                ...
                <Typography sx={{ color: TEXT.body, fontSize: '1.05rem', lineHeight: 1.7 }}>
                  {service.pain}
                </Typography>
              </Box>
            </Reveal>
            <Reveal direction="left">
              <Box sx={{ p: 4, height: '100%', borderRadius: 4, background: '#14bb870d', border: '1px solid #14bb8733' }}>
      ...
      <Box sx={{ background: SURFACE.mint, py: { xs: 8, md: 12 } }}>
        <Container maxWidth="lg">
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: { xs: 5, md: 8 } }}>
            <Box>
              <Reveal>
                <Typography variant="h4" sx={{ fontWeight: 800, mb: 3, color: TEXT.heading }}>
                  What we own for you
                </Typography>
              </Reveal>
                ...
                {service.whatWeDo.map((item, i) => (
                  <Reveal key={item} delay={i * 0.06}>
                    <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
                      ...
                      <Typography sx={{ color: TEXT.body, fontSize: '1rem', lineHeight: 1.6 }}>
                        {item}
                      </Typography>
                    </Box>
                  </Reveal>
            ...
            <Reveal direction="left">
              <Box sx={{ p: 4, borderRadius: 4, background: CARD.bg, border: CARD.border, boxShadow: CARD.shadow }}>
                <Typography variant="h6" sx={{ fontWeight: 700, mb: 2.5, color: TEXT.heading }}>
                  What you get
                </Typography>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.2 }}>
                  {service.deliverables.map((d) => (
                    ...
                    >
                      ...
                      <Typography sx={{ color: TEXT.body, fontSize: '0.95rem' }}>{d}</Typography>
                    </Box>
                  ))}
                </Box>
              </Box>
      ...
      <Box sx={{ background: SURFACE.sky, py: { xs: 8, md: 12 } }}>
        <Container maxWidth="lg">
          <Reveal>
            <Typography variant="h4" sx={{ fontWeight: 800, textAlign: 'center', mb: 5, color: TEXT.heading }}>
            ...
            {others.map((s, i) => (
              <Reveal key={s.slug} delay={i * 0.1}>
                <TiltCard max={10} sx={{ height: '100%', borderRadius: 4 }}>
                  ...
                  >
                    <Box sx={{ width: 48, height: 48, borderRadius: 2, display: 'grid', placeItems: 'center', color: s.color, background: `${s.color}14`, mb: 2 }}>
                      <ServiceIcon name={s.icon} />
                    </Box>
                    <Typography sx={{ fontWeight: 700, color: TEXT.heading, mb: 0.5 }}>{s.shortName}</Typography>
                    <Typography sx={{ color: TEXT.muted, fontSize: '0.85rem', lineHeight: 1.5, mb: 1.5 }}>
                      {s.tagline}
                    </Typography>
