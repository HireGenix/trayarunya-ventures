...
// Classic AnswerThePublic interrogatives used to cluster questions.
const QWORDS = [
  'what',
  'why',
  'how',
...
/** Word-wrap a string into up to `maxLines` lines of at most `maxChars` each. */
function wrapLabel(text: string, maxChars: number, maxLines: number): string[] {
  ...
  if (lines.length === maxLines) {
    // signal there was more text
    const last = lines[maxLines - 1];
    const consumed = lines.join(' ').length;
    if (consumed < text.trim().length) {
      lines[maxLines - 1] = truncate(last, maxChars - 1).replace(/…?$/, '…');
    }
  }
  return lines;
}

/** AnswerThePublic-style radial "content cluster web". */
function ClusterWheel({
  topic,
  clusters,
  activeKey,
  onSelect,
  ...
  // Generous padded canvas so wrapped labels never clip at the edges.
  const W = 1200;
  const H = 1200;
  const cx = W / 2;
  const cy = H / 2;
    ...
    >
      ...
      {clusters.map((c, i) => {
        ...
        return (
          <g key={c.key} opacity={dim ? 0.15 : 1} style={{ transition: 'opacity .2s' }}>
            ...
            {/* leaves spread evenly around the full outer ring (no overlap) */}
            {showLeaves &&
              leaves.map((leaf, j) => {
                const ang = -Math.PI / 2 + (2 * Math.PI * j) / leaves.length;
                const lx = cx + leafR * Math.cos(ang);
                const ly = cy + leafR * Math.sin(ang);
            ...
            <g style={{ cursor: 'pointer' }} onClick={() => onSelect(isActive ? null : c.key)}>
              <circle
                ...
                strokeWidth={3}
              />
              <text x={hx} y={hy - 2} textAnchor="middle" fontSize={13} fontWeight={700} fill="#fff">
                {truncate(c.label, 10)}
              </text>
              <text x={hx} y={hy + 13} textAnchor="middle" fontSize={11} fill="#fff" opacity={0.85}>
                {c.items.length}
              </text>
            </g>
          </g>
      ...
      <g style={{ cursor: 'pointer' }} onClick={() => onSelect(null)}>
        <circle
          ...
          strokeWidth={4}
        />
        <text x={cx} y={cy - 4} textAnchor="middle" fontSize={14} fontWeight={800} fill="#fff">
          <tspan x={cx}>{truncate(topic, 16)}</tspan>
        </text>
        <text x={cx} y={cy + 16} textAnchor="middle" fontSize={11} fill="#fff" opacity={0.85}>
          content clusters
        </text>
      </g>
    </Box>
...
export default function InsightsPage() {
  ...
  const removeInsight = async (ins: Insight) => {
    const ok = await confirm({
      ...
      message: (
        <>
          This will permanently remove <b>“{ins.text}”</b> from this research&apos;s explorer.
        </>
      ),
    });
    if (!ok) return;
    ...
    try {
      await Insights.remove(ins.id);
    } catch {
      setItems(prev); // rollback on failure
    }
  };

  // Load the workspace's research jobs (each = a client / research unit).
  useEffect(() => {
    ...
    Research.list()
      .then((list) => {
        setJobs(list);
        setJobId((prev) => prev || list[0]?.id || '');
      })
      .catch(() => setJobs([]))
      .finally(() => setLoadingJobs(false));
  }, [activeWorkspace]);

  // Load insights scoped to the selected research job only.
  useEffect(() => {
    if (!jobId) {
      setItems([]);
      return;
    ...
    <Stack spacing={3}>
      <Card>
        <CardContent sx={{ p: 3 }}>
          ...
          <Typography color="text.secondary" sx={{ mb: 2 }}>
            Pick a client&apos;s research to see its AnswerThePublic-style content cluster web —
            questions, topics and angles mined from that research only.
          </Typography>
            ...
            <TextField
              ...
              helperText={
                !loadingJobs && jobs.length === 0
                  ? 'No research yet — run a research job first.'
      ...
      {loadingJobs || loading ? (
        ...
        <Typography color="text.secondary">
          Select a client / research above to explore its insights.
        </Typography>
      ) : items.length === 0 ? (
        <Typography color="text.secondary">
          No insights for this research yet. Open it in Research and run the job to populate the
          explorer.
        </Typography>
      ) : (
        <>
            ...
            <CardContent sx={{ p: { xs: 1.5, sm: 3 } }}>
              ...
              >
                ...
                <Typography variant="caption" color="text.secondary">
                  Click a cluster to expand its questions &amp; filter the list · click the centre to
                  reset
                </Typography>
              </Stack>
              ...
              <ClusterWheel
                topic={selectedJob?.topic || 'Research'}
                clusters={clusters}
